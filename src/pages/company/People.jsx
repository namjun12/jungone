...
const People = () => {
   ...
   return (
      <PeopleWrap className='container xl:pt-180 xl:mb-120'>
         <div>
            <div>
               <Title02>인재상</Title02>
               <p className='text-center xl:leading-32 xl:text-20 xl:mt-32'>공간정원은 열린 마음과 노력하는 자세를 통해 함께 성장할 인재를 기다리고 있습니다.</p>
            </div>
               ...
               {peopleInfo.map((peopleInfo, index) => (
                  <li className='item relative xl:p-40'>
                     <div>
                        ...
                        <p
                           className='xl:leading-24 xl:text-16 xl:mt-24'
                           dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(peopleInfo.desc) }}
                        />
                     </div>
                     <span className='num leading-1em xl:text-100 font-bold text-pointColor04'>0{index + 1}</span>
                  </li>
               ))}
            </ul>
         </div>
         <div className='process xl:mt-120'>
            <div>
               <p className='leading-1em text-center xl:text-18 text-pointColor01'>채용 과정</p>
               <Title01 className='text-center xl:mt-16'>공간정원과 함께 할 인재를 기다립니다.</Title01>
            </div>
            <img className='main-img xl:mt-48' src={image20} alt="" />
         </div>
      </PeopleWrap>
   )
}
